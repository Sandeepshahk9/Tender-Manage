...
if (isset($_POST['changepassword'])) {
    ...
    } elseif (strlen($_POST['oldpass']) < 8 || strlen($_POST['newpass']) < 8 || strlen($_POST['conpass']) < 8) {
        echo '<script>location.href="change-password.php?msg=2";</script>';
    } else {
        if ($_POST['newpass'] == $_POST['conpass']) {
            $sql = mysql_query("select * from sub_admin where password = '".$_POST["oldpass"]."' AND id='" . $_SESSION["sub_admin_uniq_user_id"] . "'");
            $num = mysql_num_rows($sql);
            if ($num > 0) {
                mysql_query("update sub_admin set password = '" .$_POST["newpass"]. "', spassword='" . $_POST["newpass"] . "' where id = '" . $_SESSION["sub_admin_uniq_user_id"] . "'");
