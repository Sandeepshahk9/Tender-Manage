...
function checkpass($username,$password)
	{
	    $sql = mysql_query("SELECT * FROM sub_admin WHERE username = '".$username."' AND password = '".$password."' AND status='1'");
		$res = mysql_fetch_array($sql);
		 $num = mysql_num_rows($sql);
	if($num==1)
	 {
		
	$_SESSION['sub_admin_uniq_user_id'] = $res['id'];
	$_SESSION['sub_admin_type'] = $res['usertype'];
	...
	{
	 	echo "<script>window.location='index.php?wrong username or password';</script>";
	   }
	}
function logout()
	{
		session_start();
		unset($_SESSION['sub_admin_uniq_user_id']); 
		
        unset($_SESSION['sub_admin_type'] );		
 		session_unset();
...
$der=strtolower($admina["username"]);
    ...
    for($i=0; $i < $max_number; $i++)
    {
      ...
      $row=mysql_fetch_array($str);
        ...
        $mail->Subject = 'Reply | Tendertm.com';
        $mail->From  = $der.'@tendertm.com';
        $mail->Body = '<tr>'.$msg.'</tr>'; 
        $mail->Send();
        $mail->ClearAddresses();
...
function select_table($table,$order){
 ...
 $s = strtr(base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256, md5($superkey), serialize($stringArray), MCRYPT_MODE_CBC, md5(md5($superkey)))), '+/=', '-_,');
 return $s;

 }

 function decryptStringArray ($stringArray) {
 $s = unserialize(rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5($superkey), base64_decode(strtr($stringArray, '-_,', '+/=')), MCRYPT_MODE_CBC, md5(md5($superkey))), "\0"));
 return $s;
