				...
				$end=$plan['plan_start_date'];
					     ...
					     $stat=explode(",",$plan['state']) ; foreach($stat as $san) { 
								 ...
								 ///////////////////li code///////////////////////////////////////////// 
								  ...
								  $str="select id from tender where (added_date between '".$end."' AND '".$start."' ) AND sub_category='".$key."' AND state='".$san."' ";
...
$numub=$count;
   ...
   $szlb="select * from tender where (added_date between '".$end."' AND '".$start."' ) AND ";			
					$sub='';				
	 $cat=explode(",",$plan['sub_category']) ; foreach($cat as $key) { 
