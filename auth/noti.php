	...
	<thead>
			...
			<tbody>
			  ...
			  <?php 
                        ...
                        $str="select id from noti_table where user_id='".$_SESSION['user_id']."'  ";
...
if($numub>0)
	...
	{ 
	 ...
	 $sqlb="select * from noti_table where user_id='".$_SESSION['user_id']."'  ";	
   ...
   while ($zaw=mysql_fetch_array($sqlub)){
    ...
    $sa=mysql_query("select * from tender where id='".$zaw['tender_id']."' order by last_date DESC ");


	  while($custob=mysql_fetch_array($sa))

	{
			     ...
			     $fav=mysql_query("select id from fav_table where user_id='".$_SESSION['user_id']."' AND tender_id='".$custob['id']."'");
			
			     $favv=mysql_fetch_array($fav);
			     if(!empty($favv)){
			     ?>
			  <tr>
			      <td><a href="fav_f.php?<?php echo 'dct&d='.$favv['id'].'&p='.$url ; ?>"><i  style="color:#f00" class="fa fa-star fa-2x" aria-hidden="true"></i></a></td>
				  <?php  } 

                  else {  	?>
			     	<td><a href="fav_f.php?<?php echo 'act&u='.$_SESSION['user_id'].'&t='.$custob['id'].'&p='.$url ; ?>"><i  class="fa fa-star fa-2x" aria-hidden="true"></i></a></td>
			
			       <?php     }    ?>
			 
			 
				  <td><a href="noti_f.php?<?php echo 'dct&d='.$zaw['id'].'&p='.$url ; ?>"><i style="color:#f00" class="fa fa-bell fa-2x" aria-hidden="true"></i></a></td>
