				...
				$end=$plan['plan_start_date'];
					     ...
					     $stat=explode(",",$plan['state']) ; foreach($stat as $san) { 
								 ...
								 ///////////////////li code///////////////////////////////////////////// 
								  ...
								  $str="select id from tender where (added_date between '".$end."' AND '".$start."' ) AND sub_category='".$cat."' AND state='".$san."' ";
...
$pagename="cat_result.php?cat=".$cat."&"; 

$numub=$count;
									
				

   $szlb="select * from tender where (added_date between '".$end."' AND '".$start."' ) AND sub_category='".$cat."'";			
...
if($numub>0)
	...
	$sqlgb = $sqlb." ORDER BY last_date DESC LIMIT ".$pagestart.",".$pageentry;
