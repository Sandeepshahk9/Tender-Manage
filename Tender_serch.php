       ...
       if(isset($_REQUEST['adsearch'])){ 
							   ...
							   $sqlb="select * from tender where status='1' AND last_date>='".date('Y-m-d')."' ";
							     ...
							     if(!empty($_REQUEST['stateid'])){
								  ...
								  $sqlb=$sqlb."AND state='".$_REQUEST['stateid']."'";
								 
								 }
								  if(!empty($_REQUEST['cityid'])){
								 
								  $sqlb=$sqlb."AND city='".$_REQUEST['cityid']."'";
								 
								 }
								  if(!empty($_REQUEST['t_min'])){
								 
								  $sqlb=$sqlb."AND price>='".$_REQUEST['t_min']."'";
								 
								 }
								 if(!empty($_REQUEST['t_max'])){
								 
								  $sqlb=$sqlb."AND price=<'".$_REQUEST['t_max']."'";
								 
								 }
								  if(!empty($_REQUEST['e_min'])){
								 
								  $sqlb=$sqlb."AND emd>='".$_REQUEST['e_min']."'";
								 
								 }
                                  if(!empty($_REQUEST['e_max'])){
								 
								  $sqlb=$sqlb."AND emd=<'".$_REQUEST['e_max']."'";
								 
								 }
                                  if(!empty($_REQUEST['pub_date_from'])){
								 
								  $sqlb=$sqlb."AND pub_date=<'".$_REQUEST['pub_date_from']."'";
								 
								 }

                                   if(!empty($_REQUEST['pub_date_to'])){
								 
								  $sqlb=$sqlb."AND pub_date>='".$_REQUEST['pub_date_to']."'";
								 
								 }

                                  if(!empty($_REQUEST['close_from'])){
								 
								  $sqlb=$sqlb."AND last_date>='".$_REQUEST['close_from']."'";
								 
								 }
								  if(!empty($_REQUEST['close_to'])){
								 
								  $sqlb=$sqlb."AND last_date>='".$_REQUEST['close_to']."'";
								 
								 }

								  if(!empty($_REQUEST['type'])){
								 
								  $sqlb=$sqlb."AND 	requirement_cat='".$_REQUEST['type']."'";
								 
								 }
								 
								 if(!empty($_REQUEST['company'])){
								 
								  $sqlb=$sqlb."AND 	company='".$_REQUEST['company']."'";
...
?>                                                             
										...
										<!--	<p>Company Name: <span><?php echo $row['c_name']; ?> </span></p>  -->
											...
											<p>Value:<span><?php $price= convertCurrency($row['price']); if(!empty($price)) {  echo "₹ ".$price ; } else { echo " Ref. to document" ; } ?></span></p>	
